...
function drupalgap_toast(a,b,c){try{var d=b?b:1500;setTimeout(function(){$.mobile.loading("show",{textVisible:!0,html:a});var b=setInterval(function(){$.mobile.loading("hide");clearInterval(b)},d)},c?c:750)}catch(e){console.log("drupalgap_toast - "+e)}}function drupalgap_load_blocks(){try{drupalgap.blocks=module_invoke_all("block_info")}catch(a){console.log("drupalgap_load_blocks - "+a)}}
...
function drupalgap_path_has_entity_arg(a){try{return 1<a.length&&("comment"==a[0]||"file"==a[0]||"node"==a[0]||"taxonomy"==a[0]&&("vocabulary"==a[1]||"term"==a[1])||"user"==a[0]||"item"==a[0])?!0:!1}catch(b){console.log("drupalgap_path_has_entity_arg - "+b)}}function drupalgap_remove_page_from_dom(a){try{$("#"+a).empty().remove()}catch(b){console.log("drupalgap_remove_page_from_dom - "+b)}}function drupalgap_restart(){try{location.reload()}catch(a){console.log("drupalgap_restart - "+a)}}
...
function(a,b,c,d){try{if(0==b.length){if(e.empty_callback){var f=window[e.empty_callback];f(k)}}else{var g=[],n=e.value,p=e.label,q;for(q in b)if(b.hasOwnProperty(q)){var r=b[q],s=null,s=c?r[d]:r,u={value:s[n],label:s[p]};g.push(u)}if(0!=g.length){e.items=g;var v=_theme_autocomplete_prepare_items(e);for(q in v)v.hasOwnProperty(q)&&(u=v[q],m+="<li>"+u+"</li>");h.html(m);h.listview("refresh");h.trigger("updatelayout")}}e.finish_callback&&(f=window[e.finish_callback],f(k))}catch(w){console.log("_theme_autocomplete_success_handlers["+
a+"] - "+w)}},a=null,a=e.custom?e.handler?e.handler:e.field_info_field.settings.handler?e.field_info_field.settings.handler:"views":e.field_info_field.settings.handler,a){case "views":case "og":var n=e.path+"?"+e.filter+"="+encodeURIComponent(k);e.params&&(n+="&"+e.params);views_datasource_get_view_result(n,{success:function(a){var b=!0;e.custom&&(b=!1);var c=null,c=b?a[a.view.root]:a;(0,_theme_autocomplete_success_handlers[d])(d,c,b,a.view.child)}});break;case "base":var q=e.field_info_field.settings,
...
break}u=window[e.entity_type+"_index"];n=null;if(e.query)n=e.query;else{var n={parameters:{},parameters_op:{}},v=[entity_primary_key(e.entity_type),entity_primary_key_title(e.entity_type)];"taxonomy_term"==e.entity_type&&e.vid&&(n.parameters.vid=e.vid);n.fields=v;n.parameters[e.filter]="%"+k+"%";n.parameters_op[e.filter]="like"}u.apply(null,[n,{success:function(a){(0,_theme_autocomplete_success_handlers[d])(d,a,!1,null)}}]);break;default:a||console.log("WARNING - _theme_autocomplete - no handler provided")}}else console.log('_theme_autocomplete - A "value" and/or "label" was not supplied.')}}catch(w){console.log("_theme_autocomplete - "+
w)}}function _theme_autocomplete_prepare_items(a){try{var b=[];a.items&&(b=a.items);var c=[];if(0<b.length)for(var d in b)if(b.hasOwnProperty(d)){var e=b[d],f="",g="";"string"===typeof e?g=f=e:(f=e.value,g=e.label);var h=l(g,null,{attributes:{value:f,onclick:"_theme_autocomplete_click('"+a.attributes.id+"', this, '"+a.autocomplete_id+"')"}});c.push(h)}return c}catch(k){console.log("_theme_autocomplete_prepare_items - "+k)}}
...
function drupalgap_get_bundle(a,b){try{return console.log("WARNING - drupalgap_get_bundle() is deprecated, use entity_get_bundle() instead!"),entity_get_bundle(a,b)}catch(c){console.log("drupalgap_get_bundle - "+c)}}
...
function _drupalgap_form_render_elements(a){try{var b="",c="",d=[],e;for(e in a.elements)if(a.elements.hasOwnProperty(e)){var f=a.elements[e];f.name||(f.name=e);if(drupalgap_form_element_access(f))if(f.is_field&&"undefined"!==typeof f.field_info_instance.widget.weight)d[f.field_info_instance.widget.weight]=_drupalgap_form_render_element(a,f);else{var g=null;a.bundle&&(g=a.bundle,"comment"==a.entity_type&&-1!=a.bundle.indexOf("comment_node_")&&(g=a.bundle.replace("comment_node_","")));if("undefined"!==
typeof f.weight)if(d[f.weight]){var h="WARNING: _drupalgap_form_render_elements - the weight of "+f.weight+" for "+f.name+" is already in use by "+d[f.weight].name;console.log(h);var k=_drupalgap_form_render_element(a,f);"undefined"!==typeof k&&(b+=k)}else d[f.weight]=_drupalgap_form_render_element(a,f);else if(a.entity_type&&g&&"undefined"!==typeof drupalgap.field_info_extra_fields[g][e]&&"undefined"!==typeof drupalgap.field_info_extra_fields[g][e].weight){var m=drupalgap.field_info_extra_fields[g][e].weight;
d[m]?(h="WARNING: _drupalgap_form_render_elements - the weight of "+m+" for "+f.name+" is already in use by "+d[m].name,console.log(h),k=_drupalgap_form_render_element(a,f),"undefined"!==typeof k&&(b+=k)):d[m]=_drupalgap_form_render_element(a,f)}else k=_drupalgap_form_render_element(a,f),"undefined"!==typeof k&&(b+=k)}}if(!empty(d)){for(m in d)c+=d[m]+"\n";b=c+"\n"+b}if(a.buttons&&0!=a.buttons.length)for(e in a.buttons)if(a.buttons.hasOwnProperty(e)){var n=a.buttons[e];if(drupalgap_form_element_access(n)){var q=
{type:"button",id:drupalgap_form_get_element_id(e,a.id)};n.attributes&&$.extend(q,n.attributes);b+="<button "+drupalgap_attributes(q)+'">'+n.title+"</button>"}}return b}catch(p){console.log("_drupalgap_form_render_elements - "+p)}}
...
var r={"class":drupalgap_form_get_element_container_class(d)};e&&(c+="<div "+drupalgap_attributes(r)+">");"submit"==b.type||"hidden"==b.type||"undefined"!==typeof b.title_placeholder&&b.title_placeholder||(c=b.is_field?c+h:c+theme("form_element_label",{element:b}));""!=g&&(c+=g);b.description&&"hidden"!=b.type&&(c+='<div class="description">'+t(b.description)+"</div>");e&&(c+="</div>");b.suffix&&(c+=b.suffix);return c}catch(s){console.log("_drupalgap_form_render_element - "+s)}}
...
!1;else"user"==a&&(a=0!=Drupal.user.uid?"user/"+Drupal.user.uid:"user/login");return a}catch(g){console.log("drupalgap_goto_prepare_path - "+g)}}
function drupalgap_back(){try{var a=$(".ui-page-active").attr("id");if(a==drupalgap.settings.front){var b=t("Exit")+" "+drupalgap.settings.title+"?";drupalgap.settings.exit_message&&(b=drupalgap.settings.exit_message);drupalgap_confirm(b,{confirmCallback:_drupalgap_back_exit})}else"_drupalgap_splash"!=a&&_drupalgap_back()}catch(c){console.log("drupalgap_back"+c)}}
...
function template_preprocess_page(a){try{"undefined"===typeof a.attributes&&(a.attributes={}),a.attributes["data-role"]="page",module_invoke_all("preprocess_page",a),drupalgap.page.variables=a}catch(b){console.log("template_preprocess_page - "+b)}}
function template_process_page(a){try{var b=drupalgap_path_get();drupalgap.output=menu_execute_active_handler();var c=drupalgap_get_page_id(b),d=$("#"+c),e=$(d).html();if(e){for(var f in drupalgap.theme.regions)if(drupalgap.theme.regions.hasOwnProperty(f)){var g=drupalgap.theme.regions[f],b={};$.extend(!0,b,g);e=e.replace("{:"+g.name+":}",drupalgap_render_region(b))}$(d).html(e);module_invoke_all("post_process_page",a)}}catch(h){console.log("template_process_page - "+h)}}
...
function drupalgap_render_page(){try{module_invoke_all("page_build",drupalgap.output);var a=drupalgap.output,b=$.type(a),c="";if("string"===b)c=a;else if("object"===b){b=["theme","view_mode","language"];if(a.theme&&drupalgap.theme_registry[a.theme]){var d=drupalgap.theme_registry[a.theme],e=a.theme.replace(/_/g,"-")+".tpl.html",f=d.path+"/"+e;if(drupalgap_file_exists(f)){var g=drupalgap_file_get_contents(f);if(g){var h=drupalgap_get_placeholders_from_html(g);if(h)for(var k in h)if(h.hasOwnProperty(k)){var m=
h[k],d="";if(a[m]){var n=a[m];a[m].markup?d=a[m].markup:a[m].theme&&(d=theme(a[m].theme,n));delete a[m]}g=g.replace("{:"+m+":}",d)}c+=g}else console.log("drupalgap_render_page - failed to get file contents ("+f+")")}else console.log("drupalgap_render_page - template file does not exist ("+f+")")}for(n in a)if(a.hasOwnProperty(n)){var q=a[n];-1==$.inArray(n,b)&&(c+=theme(q.theme,q))}}return c}catch(p){console.log("drupalgap_render_page - "+p)}}
...
"#"+menu_container_id(m.options.popup_delta)):"undefined"===typeof m.options.attributes["data-role"]&&(m.options.attributes["data-role"]="button");"notext"==m.options.attributes["data-iconpos"]&&(s="&nbsp;");var n=drupalgap_link_get_class(r),q=menu_region_link_get_side(n),v=l(s,u,m.options);"left"==q?h+=v:"right"==q&&(k+=v)}}if(1<f){var w={"data-type":"horizontal","data-role":"controlgroup","class":"ui-btn-left"};h="<div "+drupalgap_attributes(w)+">"+h.replace(/ui-btn-left/g,"")+"</div>"}1<g&&(w=
{"data-type":"horizontal","data-role":"controlgroup","class":"ui-btn-right"},k="<div "+drupalgap_attributes(w)+">"+k.replace(/ui-btn-right/g,"")+"</div>");c+=h+k}var f={block_count:0,block_menu_count:0},y=drupalgap.settings.blocks[drupalgap.settings.theme][a.name],x;for(x in y)if(y.hasOwnProperty(x)){var z=y[x];"_prefix"!=x&&"_suffix"!=x&&(c+=drupalgap_block_render(a,b,x,z,f))}if(in_array(a.attributes["data-role"],["header","footer"])&&(0==f.block_count&&0<d||0==f.block_count-f.block_menu_count)||
...
function theme_video(a){try{return a.path&&(a.attributes.src=a.path),a.alt&&(a.attributes.alt=a.alt),a.title&&(a.attributes.title=a.title),"undefined"!==typeof device&&"iOS"==device.platform&&"undefined"===typeof a.attributes["webkit-playsinline"]&&(a.attributes["webkit-playsinline"]=""),"<video "+drupalgap_attributes(a.attributes)+"></video>"}catch(b){console.log("theme_video - "+b)}}
function theme_image_style(a){try{return a.path=image_style_url(a.style_name,a.path),theme_image(a)}catch(b){console.log("theme_image - "+b)}}function theme_item(a){try{var b="",c;for(c in a.item)a.item.hasOwnProperty(c)&&(b+="<h2>"+a.model.fields[c].title+"</h2><p>"+a.item[c]+"</p>");return b}catch(d){console.log("theme_item - "+d)}}
...
function image_field_formatter_view(a,b,c,d,e,f,g){try{a={};empty(f)&&d.settings.default_image&&(f=[{uri:d.settings.default_image_uri}]);if(!empty(f))for(var h in f)if(f.hasOwnProperty(h)){var k=f[h],m=empty(g.settings.image_style)?"image":"image_style",n={theme:m,alt:k.alt,title:k.title};"image_style"==m?(n.style_name=g.settings.image_style,n.path=k.uri):n.path=drupalgap_image_path(k.uri);a[h]=n}return a}catch(q){console.log("image_field_formatter_view - "+q)}}
...
c;m=!0;break}if(m){var n=g[0];"taxonomy"!=g[0]||"vocabulary"!=g[1]&&"term"!=g[1]||(n=g[0]+"_"+g[1]);n+="_load";if(drupalgap_function_exists(n)){var q=window[n],p=parseInt(g[k]);"item"==g[0]?(entity=q(g[1],g[2],p),d(entity)):(g=!1,"edit"==arg(2)&&(g=!0),q(p,{reset:g,success:d}))}else console.log("menu_block_view_pageshow - load function not implemented! "+n)}else d(null)}else d(null)}}else{q=!1;if(drupalgap.menus[d]&&drupalgap.menus[d].links){q=drupalgap.menus[d];p=[];for(k in q.links)if(q.links.hasOwnProperty(k)){h=
jQuery.extend(!0,{},q.links[k]);h.options?h.options.attributes||(h.options.attributes={}):h.options={attributes:{}};h.options.attributes["class"]||(h.options.attributes["class"]="");var r=h.options.attributes["class"];h.path==c&&(-1==r.indexOf("ui-btn")&&(r+=" ui-btn"),-1==r.indexOf("ui-btn-active")&&(r+=" ui-btn-active"),-1==r.indexOf("ui-state-persist")&&(r+=" ui-state-persist"));h.options.attributes["data-icon"]&&-1==r.indexOf(h.options.attributes["data-icon"])&&(r+=" ui-icon-"+h.options.attributes["data-icon"]+
" ");h.options.attributes["class"]=r+" ";p.push(l(t(h.title),h.path,h.options))}0<p.length&&(g=null,q.options&&q.options.attributes&&(g=drupalgap.menus[d].options.attributes),b=theme("item_list",{items:p,attributes:g}))}$("#"+a.container_id).html(b).trigger("create");p=!1;q&&"undefined"!==typeof q.options&&"undefined"!==typeof q.options.wrap&&q.options.wrap&&(p=!0);p||$("#"+a.container_id).children().unwrap()}}catch(s){console.log("menu_block_view_pageshow - "+s)}}
function menu_install(){try{var a=menu_list_system_menus(),b;for(b in a)a.hasOwnProperty(b)&&menu_save(a[b])}catch(c){console.log("menu_install - "+c)}}function menu_popup_get_default_options(){return{attributes:{"data-role":"listview"},wrap:!0,wrap_options:{attributes:{"data-role":"popup"}}}}function menu_region_link_get_data(a){try{var b=null;return b="undefined"===typeof a.region?a:a.region}catch(c){console.log("menu_region_link_get_data - "+c)}}
function menu_region_link_get_side(a){try{var b="left";-1!=a.indexOf("ui-btn-right")&&(b="right");return b}catch(c){console.log("menu_region_link_get_side - "+c)}}function menu_save(a){try{drupalgap.menus[a.menu_name]=a}catch(b){console.log("menu_save - "+b)}}function menu_load(a){try{return"undefined"!==typeof drupalgap.menus[a]?drupalgap.menus[a]:null}catch(b){console.log("menu_load - "+b)}}
function menu_container_id(a,b){try{var c=null,c=b?b:drupalgap_get_page_id();return c+"_menu_"+a}catch(d){console.log("menu_container_id - "+d)}}function collection_list_page(a,b){try{var c={collection_list:{theme:"jqm_item_list",title:t("Collection")}},d=[],e=collection_load(a,b);if(e){for(var f in e)e.hasOwnProperty(f)&&d.push(l(e[f].name,"mvc/item/"+a+"/"+b+"/"+f));c.collection_list.items=d}return c}catch(g){console.log("collection_list_page - "+g)}}
function collection_load(a,b){try{return JSON.parse(window.localStorage.getItem(mvc_get_collection_key("collection",a,b)))}catch(c){console.log("collection_load - "+c)}}function collection_save(a,b,c){try{window.localStorage.setItem(mvc_get_collection_key("collection",a,b),JSON.stringify(c))}catch(d){console.log("collection_save - "+d)}}function mvc_get_collection_key(a,b,c){return"mvc_"+a+"_"+b+"_"+c}
...
theme("logout"):"";case "title":return'<h1 id="'+system_title_block_id(drupalgap_path_get())+'" class="page-title"></h1>';case "powered_by":return'<p style="text-align: center;">'+t("Powered by")+": "+l("DrupalGap","http://www.drupalgap.org",{InAppBrowser:!0})+"</p>";case "help":return l("Help","http://www.drupalgap.org/support");default:return""}}catch(d){console.log("system_block_info - "+d)}}
...
function user_logout_callback(){return"<p>"+t("Logging out")+"...</p>"}function user_logout_pagechange(){try{user_logout({success:function(a){drupalgap_goto(drupalgap.settings.front,{reloadPage:true})}})}catch(a){console.log("user_logout_pagechange - "+a)}}
...
function taxonomy_views_exposed_filter(a,b,c,d,e){try{if("textfield"==d.options.type)c.type="autocomplete",c.remote=!0,c.custom=!0,c.handler="index",c.entity_type="taxonomy_term","undefined"!==typeof d.options.vocabulary&&(c.vid=taxonomy_vocabulary_get_vid_from_name(d.options.vocabulary)),c.value="name",c.label="name",c.filter="name";else{c.type="hidden";for(var f in e.settings.allowed_values)if(e.settings.allowed_values.hasOwnProperty(f)){var g={required:c.required,render_input_element:!1,attributes:{id:c.options.attributes.id},
field_info_field:e,field_info_instance:{widget:{type:"options_select"}},exposed:!0};empty(d.value)||(g.value=parseInt(d.value[0]),g.attributes.value=g.value);a="";empty(c.title)||(a+=theme("form_element_label",{element:c}));a+=theme("taxonomy_term_reference",g);c.children.push({markup:a})}}}catch(h){console.log("taxonomy_views_exposed_filter - "+h)}}var _views_embedded_views={};
function views_embedded_view_get(a,b){try{return _views_embedded_views[a]?b?_views_embedded_views[a][b]:_views_embedded_views[a]:null}catch(c){console.log("views_embedded_view_get - "+c)}}function views_embedded_view_set(a,b,c){try{_views_embedded_views[a]||(_views_embedded_views[a]={}),_views_embedded_views[a][b]=c}catch(d){console.log("views_embedded_view_set - "+d)}}
function views_embedded_view_delete(a,b){try{if(!_views_embedded_views[a])return!1;b?delete _views_embedded_views[a][b]:delete _views_embedded_views[a];return!0}catch(c){console.log("views_embedded_view_delete - "+c)}}
...
attributes:{id:a.id+"-reset",onclick:"views_exposed_form_reset()"}});return a}catch(n){console.log("views_exposed_form - "+n)}}
function views_exposed_form_submit(a,b){try{var c=a.variables.page_id,d="",e;for(e in b.values)if(b.values.hasOwnProperty(e)){var f=b.values[e];empty(f)||(d+=e+"="+encodeURIComponent(f)+"&")}empty(d)||(d=d.substr(0,d.length-1));var g=views_embedded_view_get(c,"exposed_filter_query");if(g){if(g==d)return;-1!=a.variables.path.indexOf("&"+g)&&(a.variables.path=a.variables.path.replace("&"+g,""))}views_embedded_view_set(c,"exposed_filter_query",d);views_embedded_view_set(c,"exposed_filter_reset",!0);
a.variables.path+="&"+d;a.variables.page=0;views_embedded_view_set(c,"exposed_filter_submit_variables",a.variables);_theme_view(a.variables)}catch(h){console.log("views_exposed_form_submit - "+h)}}
function views_exposed_form_reset(){try{var a=drupalgap_get_page_id(),b=views_embedded_view_get(a,"exposed_filter_submit_variables");b.path=b.path.replace("&"+views_embedded_view_get(a,"exposed_filter_query"),"");b.page=0;views_embedded_view_set(a,"exposed_filter_submit_variables",b);views_embedded_view_set(a,"exposed_filter_reset",!1);views_embedded_view_set(a,"exposed_filter_query",null);_theme_view(b)}catch(c){console.log("views_exposed_form_reset - "+c)}}
...
function views_embed_view(a,b){try{views_datasource_get_view_result(a,{success:function(a){try{if(views_embedded_view_set(b.page_id,"results",a),views_embedded_view_set(b.page_id,"options",b),b.success){b.results=a;var c=theme("views_view",b);b.success(c)}}catch(f){console.log("views_embed_view - success - "+f)}},error:function(a,c,f){try{views_embedded_view_set(b.page_id,"results",null),b.error&&b.error(a,c,f)}catch(g){console.log("views_embed_view - error - "+g)}}})}catch(c){console.log("views_embed_view - "+
c)}}
function theme_views_view(a){try{var b="",c=views_embedded_view_get(a.page_id,"results");if(!c)return b;a.format||(a.format="unformatted_list");var d=c.view.root,e=c.view.child;a.title&&(b+=theme("header",{text:a.title,attributes:a.title_attributes?a.title_attributes:null}),"unformatted"!=a.format&&(b+=theme("views_spacer",null)));var f="";"undefined"!==typeof c.view.exposed_data&&(f=drupalgap_get_form("views_exposed_form",{exposed_data:c.view.exposed_data,exposed_raw_input:c.view.exposed_raw_input,filter:c.view.filter,
variables:a}));var g="#"+a.page_id+" #"+a.attributes.id;views_embedded_view_set(a.page_id,"selector",g);var h=module_exists("views_litepager");if(0==c.view.count&&!h||h&&null==c.view.pages&&0==c.view.count)return $(g).hide(),setTimeout(function(){$(g).trigger("create").show("fast")},100),a.empty_callback&&function_exists(a.empty_callback)?f+(0,window[a.empty_callback])(c.view):b+f;var b=b+f,k=drupalgap_views_get_result_formats(a),m=""+k.open+drupalgap_views_render_rows(a,c,d,e,k.open_row,k.close_row)+
k.close,d="";c.view.pages&&(d=theme("pager",a));c="top";"undefined"!==typeof a.pager_pos&&(c=a.pager_pos);module_exists("views_infinite_scroll")&&views_infinite_scroll_ok()?b+=m:"top"==c?(b+=d,empty(d)||(b+=theme("views_spacer",null)),b+=m):"bottom"==c?(b+=m,empty(d)||(b+=theme("views_spacer",null)),b+=d):console.log("WARNING: theme_views_view - unsupported pager_pos ("+c+")");$(g).hide();setTimeout(function(){$(g).trigger("create").show("fast")},100);return b}catch(n){console.log("theme_views_view - "+
n)}}function theme_views_spacer(a){return'<h2 class="dg_empty_list_header">&nbsp;</h2>'}
function theme_pager(a){try{var b="",c=a.results.view,d=c.page,e=c.pages,f=c.count,d=c.page,g=module_exists("views_litepager");if(0==f&&!g||g&&null==a.results.view.pages)return b;c=[];0!=d&&c.push(theme("pager_previous",a));(d!=e-1&&!g||g)&&c.push(theme("pager_next",a));if(0<c.length)var h="theme_pager_"+user_password(),b=b+("<div "+drupalgap_attributes({id:h,"class":"pager","data-role":"navbar"})+">"+theme("item_list",{items:c})+'</div><script type="text/javascript">$("#'+h+'").navbar();\x3c/script>');
return b}catch(k){console.log("theme_pager - "+k)}}function theme_pager_link(a,b){try{b.attributes||(b.attributes={});b.attributes.href="#";var c=drupalgap_attributes(b.attributes);return"<a onclick='"+_theme_pager_link_onclick(a)+"' "+c+">"+b.text+"</a>"}catch(d){console.log("theme_pager_link - "+d)}}
function _theme_pager_link_onclick(a){try{var b=$.extend({},{},a);b.results&&delete b.results;return"_theme_pager_link_click("+JSON.stringify(b)+")"}catch(c){console.log("_theme_pager_link_onclick - "+c)}}function _theme_pager_link_click(a){try{_theme_view(a)}catch(b){console.log("_theme_pager_link_click - "+b)}}
function theme_pager_next(a){try{return a.page=parseInt(a.results.view.page)+1,theme_pager_link(a,{text:"&raquo;",attributes:{"class":"pager_next"}})}catch(b){console.log("theme_pager_next - "+b)}}function theme_pager_previous(a){try{return a.page=parseInt(a.results.view.page)-1,theme_pager_link(a,{text:"&laquo;",attributes:{"class":"pager_previous"}})}catch(b){console.log("theme_pager_previous - "+b)}}
