 ...
 * Define the form.
 */
function reiscout_buy_info_points_custom_form(form, form_state) {
  try {
    form.options.attributes['style'] = 'display: none';
    form.elements['submit'] = {
      type: 'submit',
      value: 'Buy Ability to Get Property Data'
    };
    return form;
  }
  catch (error) { console.log('reiscout_buy_info_points_custom_form - ' + error); }
}
        ...
        if (result.length == 0) {
          ...
          commerce_cart_create({
            success: function(order) {
              _commerce_line_point_item_add_to_order({
                order: order,
...
function _commerce_line_point_item_add_to_order(options) {
  ...
  catch (error) { console.log('_commerce_line_point_item_add_to_order - ' + error); }
}